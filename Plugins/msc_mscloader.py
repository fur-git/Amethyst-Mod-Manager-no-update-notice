"""
MSCLoader installation wizard for My Summer Car.

Workflow:
  1. Prompt user to download MSCLoader from Nexus Mods.
  2. Auto-detect the archive in Downloads (keyword: mscloader_msc).
  3. Extract to the game folder.
  4. Run MSCLInstaller.exe via Proton.
  5. Clean up archive files.
"""
...
_ARCHIVE_KEYWORD = "mscloader_msc"
_EXE_NAME        = "MSCLInstaller.exe"
...
class MSCLoaderWizard(ctk.CTkFrame):
    ...
    def __init__(self, parent, game, log_fn=None, *, on_close=None, **_extra):
        ...
        ctk.CTkLabel(
            title_bar,
            text=f"Install MSCLoader \u2014 {game.name}",
    ...
    def _show_step_download(self):
        ...
        ctk.CTkLabel(
            self._body, text="Step 1: Download MSCLoader",
        ...
        ctk.CTkLabel(
            self._body,
            text=(
                "Click the button below to open the MSCLoader page on Nexus Mods.\n\n"
    ...
    def _scan_downloads(self):
        ...
        if found:
            ...
            self._locate_status.configure(
                text=(
                    "MSCLoader archive not found in Downloads.\n"
    ...
    def _do_extract(self):
        try:
            ...
            self._log(f"MSCLoader Wizard: extracting {archive.name} \u2192 {game_path}")

            paths = _extract_archive(archive, game_path)
            self._extracted_paths = paths
            file_count = len([p for p in paths if p.is_file()])
            self._log(f"MSCLoader Wizard: extracted {file_count} file(s).")
            ...
            self.after(300, self._show_step_mscfolder)

        except Exception as exc:
            self._set_label("_extract_status", f"Error: {exc}", color="#e06c6c")
            self._log(f"MSCLoader Wizard: extract error: {exc}")

    # ------------------------------------------------------------------
    # Step 4 — Create MSCFolder.txt
    # ------------------------------------------------------------------

    def _show_step_mscfolder(self):
        self._clear_body()

        ctk.CTkLabel(
            self._body, text="Step 4: Create MSCFolder.txt",
            font=FONT_BOLD, text_color=TEXT_MAIN,
        ).pack(pady=(0, 12))

        self._mscfolder_status = ctk.CTkLabel(
            self._body, text="Writing MSCFolder.txt\u2026",
            font=FONT_NORMAL, text_color=TEXT_DIM, justify="center", wraplength=460,
        )
        self._mscfolder_status.pack(pady=(0, 16))

        game_path = self._game.get_game_path()
        if game_path is None:
            self._set_label(
                "_mscfolder_status", "Game path not configured.", color="#e06c6c",
            )
            return

        try:
            msc_folder_txt = game_path / "MSCFolder.txt"
            msc_folder_txt.write_text(str(game_path), encoding="utf-8")
            self._log(f"MSCLoader Wizard: wrote {msc_folder_txt}")
            self._set_label(
                "_mscfolder_status",
                f"Created MSCFolder.txt \u2192 {game_path}",
                color="#6bc76b",
            )
            self.after(300, self._show_step_run)
        except Exception as exc:
            self._set_label("_mscfolder_status", f"Error: {exc}", color="#e06c6c")
            self._log(f"MSCLoader Wizard: MSCFolder.txt error: {exc}")

    # ------------------------------------------------------------------
    # Step 5 — Run MSCLInstaller.exe
    # ------------------------------------------------------------------
    ...
    def _show_step_run(self):
        ...
        ctk.CTkLabel(
            self._body, text="Step 5: Run MSCLInstaller",
            font=FONT_BOLD, text_color=TEXT_MAIN,
        ).pack(pady=(0, 12))
        ...
        self._run_status = ctk.CTkLabel(
            self._body, text="Launching MSCLInstaller\u2026",
            font=FONT_NORMAL, text_color=TEXT_DIM, justify="center", wraplength=460,
        )
    ...
    def _do_run(self, exe: Path):
        ...
        self._log(f"MSCLoader Wizard: launching {exe} via Proton")
        try:
            ...
            self._set_label(
                "_run_status",
                "MSCLInstaller is running.\nClose it when you are done, then click Done.",
                color="#6bc76b",
            )
            self.after(0, lambda: self._done_btn.configure(state="normal"))
            proc.wait()
            self._log("MSCLoader Wizard: MSCLInstaller closed.")
            self._set_label("_run_status", "MSCLInstaller finished.", color="#6bc76b")
        except Exception as exc:
            self._set_label("_run_status", f"Launch error: {exc}", color="#e06c6c")
            self._log(f"MSCLoader Wizard: launch error: {exc}")
    ...
    def _finish_cleanup(self, installer_exe: Path):
        """Remove archive, extracted files, and MSCFolder.txt, then close."""
        removed_count = 0
        ...
        # Remove all extracted files and directories (deepest first)
        ...
        # Remove MSCFolder.txt
        game_path = self._game.get_game_path()
        if game_path:
            msc_folder_txt = game_path / "MSCFolder.txt"
            if msc_folder_txt.is_file():
                try:
                    msc_folder_txt.unlink()
                    removed_count += 1
                except OSError:
                    pass

        if removed_count:
            self._log(f"MSCLoader Wizard: cleaned up {removed_count} file(s).")
