...
class CustomGamePanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            ...
            text="Route specific files to alternate destinations during deploy. "
                 ...
                 "Enable To Prefix to route the rule's destination relative to the Proton/Wine prefix root "
                 "instead of the game install root (e.g. drive_c/users/steamuser/AppData/...).",
            font=FONT_SMALL, text_color=TEXT_DIM, anchor="center",
            wraplength=WRAP,
        ).grid(row=row, column=0, sticky="ew", padx=16, pady=(0, 4))
        row += 1
        ...
        for rule_data in self._routing_rules_initial:
            if isinstance(rule_data, dict):
                ...
                self._add_routing_rule_row(
                    ...
                    to_prefix=bool(rule_data.get("to_prefix", False)),
                )
    ...
    def _add_routing_rule_row(self, dest: str = "", match_type: str = "extensions",
        ...
        if not self._routing_rules_rows:
            ...
            hdr.grid_columnconfigure(8, weight=0)
        ...
        row_frame.grid_columnconfigure(8, weight=0)
        ...
        to_prefix_var = tk.BooleanVar(value=to_prefix)
        ...
        loose_cell = ctk.CTkFrame(row_frame, fg_color="transparent")
        loose_cell.grid(row=0, column=4, padx=(4, 2), pady=4)
        ctk.CTkLabel(
            loose_cell, text="Loose only", font=FONT_SMALL, text_color=TEXT_MAIN,
        ).pack(side="left", padx=(0, 4))
        ctk.CTkSwitch(
            loose_cell, text="", variable=loose_var,
            font=FONT_SMALL, text_color=TEXT_MAIN,
            fg_color=BG_DEEP, progress_color=ACCENT, width=40,
        ).pack(side="left")

        flatten_cell = ctk.CTkFrame(row_frame, fg_color="transparent")
        flatten_cell.grid(row=0, column=5, padx=(4, 2), pady=4)
        ctk.CTkLabel(
            flatten_cell, text="Flatten", font=FONT_SMALL, text_color=TEXT_MAIN,
        ).pack(side="left", padx=(0, 4))
        ctk.CTkSwitch(
            flatten_cell, text="", variable=flatten_var,
            font=FONT_SMALL, text_color=TEXT_MAIN,
            fg_color=BG_DEEP, progress_color=ACCENT, width=40,
        ).pack(side="left")

        siblings_cell = ctk.CTkFrame(row_frame, fg_color="transparent")
        siblings_cell.grid(row=0, column=6, padx=(4, 2), pady=4)
        ctk.CTkLabel(
            siblings_cell, text="Include Siblings", font=FONT_SMALL, text_color=TEXT_MAIN,
        ).pack(side="left", padx=(0, 4))
        ctk.CTkSwitch(
            siblings_cell, text="", variable=siblings_var,
            font=FONT_SMALL, text_color=TEXT_MAIN,
            fg_color=BG_DEEP, progress_color=ACCENT, width=40,
        ).pack(side="left")

        prefix_cell = ctk.CTkFrame(row_frame, fg_color="transparent")
        prefix_cell.grid(row=0, column=7, padx=(4, 2), pady=4)
        ctk.CTkLabel(
            prefix_cell, text="To Prefix", font=FONT_SMALL, text_color=TEXT_MAIN,
        ).pack(side="left", padx=(0, 4))
        ctk.CTkSwitch(
            prefix_cell, text="", variable=to_prefix_var,
            font=FONT_SMALL, text_color=TEXT_MAIN,
            fg_color=BG_DEEP, progress_color=ACCENT, width=40,
        ).pack(side="left")
        ...
        ).grid(row=0, column=8, padx=(2, 6), pady=4)
