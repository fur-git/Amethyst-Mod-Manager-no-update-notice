...
def _fix_nonutf8_names_extracted_tree(extract_dir: str, log_fn: LogFn) -> int:
    ...
    """
    from Utils.filegraph.paths import repair_nonutf8_names
...
def prepare_archive(archive_path: str, game, profile_dir: Path, *,
    ...
    *cancel* - optional ``threading.Event``; when set the extraction is aborted
    ...
    from Utils.fomod.parser import (detect_fomod, detect_scripted_fomod,
                                    parse_module_config)
    ...
    if (fomod_result is None and not prepared.bain_subpkgs
        ...
        try:
            from Utils.re_engine.bundle import detect_re_bundle, detect_multi_mod
...
def _clean_mod_name(stem: str, game) -> str:
    ...
    name = stem
    try:
        from Utils.mods.names import _suggest_mod_names, sanitize_mod_folder_name
...
def _add_to_modlist(profile_dir: Path, mod_name: str, log_fn: LogFn,
    ...
    try:
        from Utils.ui.config import load_install_mods_disabled
