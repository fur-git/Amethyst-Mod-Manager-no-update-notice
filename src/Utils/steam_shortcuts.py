...
libraryfolders.vdf uses, so none of steam_finder's regex parsers apply. The
...
find_proton_for_game() in steam_finder work on it unchanged.
...
from Utils.steam_finder import _STEAM_CANDIDATES, _stat_sig, find_prefix
...
def build_installed_exe_index() -> "list[list[str]]":
    ...
    Mirrors the Lutris/Faugus one-pass indexes ``installed_scan`` builds.
    """
    out: list[list[str]] = []
