...
:mod:`Utils.assets.nif` can seek straight to the blocks it cares about and
...
``Utils/assets/``), the same description NifSkope is generated from. Block
...
nif.xml is BSD-licensed by the NifTools team; ``Utils/assets/nif.xml`` is an
unmodified copy.
"""
...
_XML_PATH = Path(__file__).resolve().parent / "nif.xml"
