...
def _source_paths(snapshot, profile_dir):
    result = {}
    for name, winner in snapshot.plugin_winners().items():
        if winner.mod_key == '[overwrite]':
            root = profile_dir / 'overwrite'
        elif winner.mod_key == '[root_folder]':
            root = profile_dir / 'Root_Folder'
        else:
            root = profile_dir / 'mods' / winner.mod_name
...
def _plugin_order(game, profile_dir, manifest, amethyst_state, entries, snapshot, folders, log):
    ...
    selected = {str(p.get('name', '')).lower() for p in manifest.get('plugins', [])}
    ...
    if getattr(game, 'loot_sort_enabled', False) and _loot_available() and authored:
        ...
        sources = ProfileSources(
            ...
            Path(game.get_mod_data_path()), profile_dir / 'mods', profile_dir / 'overwrite',
            profile_dir / 'Root_Folder')
        with tempfile.TemporaryDirectory(prefix='amethyst-collection-order-') as temp:
            ...
            result = sort_plugins(
                ...
                game_name=game.name, game_path=game.get_game_path(), staging_root=profile_dir / 'mods',
                ...
                plugin_winner_paths=_source_paths(snapshot, profile_dir), profile_sources=sources)
...
def reset_group_collection_load_order(game, group_dir: Path, member_name: str,
                                      manifest: dict, *, amethyst_state=None, log_fn=None) -> dict:
    ...
    with game_mutation_lock(game), groups.group_build_lock(group_dir), ExitStack() as locks:
        ...
        if groups.profile_is_locked(group_dir) or groups.profile_is_locked(member_dir):
            raise ValueError('Unlock the group and collection profile before resetting their order.')
        if game.get_deploy_active() and game.get_last_deployed_profile() in (group_dir.name, member_name):
            raise ValueError('Restore the deployed profile before resetting its collection order.')
        journal = group_dir / '.collection-order-recovery.json'
        recovery = _read_recovery(game, group_dir, journal)
        for path in sorted({group_dir, member_dir, *(path.parent for path in recovery)}):
            locks.enter_context(modlist_lock(path / 'modlist.txt'))
        if recovery:
            _restore_order_files(recovery)
            journal.unlink()
        desired = resolve_collection_mod_order(member_dir, manifest, amethyst_state)
        ...
        for name in desired:
            key, _version = groups._mod_identity_and_version(member_dir / 'mods', name)
        ...
        old_plugins = {path: _plugin_entries(game, path) for path in (member_dir, group_dir)}
        ...
        if getattr(game, 'plugin_extensions', ()):
            with _preview(game, member_dir, member_final) as (before, after):
                ...
                _validate_plugins(game, old_plugins[member_dir], new_plugins[member_dir],
                                  _source_paths(before, member_dir), _source_paths(after, member_dir))
        ...
        with tempfile.TemporaryDirectory(prefix='amethyst-group-reset-') as temp:
            for i, (path, mods) in enumerate(((member_dir, member_final), (group_dir, group_final))):
            ...
            try:
                ...
                for path in (member_dir, group_dir):
        ...
        return {'ordered': len(desired), 'group': group_dir.name, 'member': member_name}
