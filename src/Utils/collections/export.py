...
Rows come from ``Utils.profiles.export.load_rows`` plus the per-row UI flags
...
def _module_config_from_archive(archive: Path):
    ...
    from Utils.fomod.parser import parse_module_config
    xml_bytes = _module_config_bytes(archive)
    if not xml_bytes:
        return None
    try:
...
def _ini_tweak_jobs(game_name: str, profile_dir,
    ...
    installer (``Utils.collections.ini_tweaks``) and Vortex merge from.
    """
...
def _conflict_mod_rules(ordered_names: list, refs: dict, game,
    ...
    *ordered_names* is LOWEST-priority-first - the order the export rows carry,
    ...
    (``Utils.collections.reset._topo_sort_collection``): ``source after reference``
...
def _load_order_block(entries: list) -> "list | None":
    ...
    the contract ``Utils.collections.reset._resolve_collection_priorities`` consumes
...
def _plugin_rules_block(profile_dir, known_plugins=None) -> "dict | None":
    ...
    installer (``Utils.collections.reset._apply_collection_groups``) merges
    ...
    *known_plugins* - lowercase names of the plugins the collection actually
    ...
    if not path or not path.is_file():
        return None
    try:
        from Utils.plugins.userlist import parse_userlist
