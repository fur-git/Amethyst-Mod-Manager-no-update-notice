...
Ownership matching mirrors ``Utils.collections.diff``: a mod belongs to the
collection if its meta.ini ``fromCollection`` equals the slug, or (legacy/un-tagged) its
...
def _manifest_file_ids(manifest: dict) -> set[int]:
    """Set of fileIds from the manifest shape used by ``Utils.collections.diff``."""
    fids: set[int] = set()
...
def resolve_owned_mod_names(game, profile_dir: Path, record: dict) -> list[str]:
    ...
    ``Utils.collections.diff``: mods from other collections / manual installs
    are skipped.
    """
