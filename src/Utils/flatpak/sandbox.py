...
def _steam_flatpak_owns_path(path: Path) -> bool:
    ...
    if not steam_root.is_dir():
        return False
    try:
        from Utils.launchers.steam import parse_vdf_libraries, _VDF_FILENAMES
    except Exception:
        return False
    try:
...
def sandbox_app_for_game(game, game_root: Optional[Path]) -> Optional[str]:
    ...
    if _heroic_flatpak_present():
        try:
            from Utils.executables.launch import game_is_heroic_install
    ...
    if (_HOME / ".var" / "app" / LUTRIS_FLATPAK_ID).is_dir():
        try:
            from Utils.executables.launch import game_is_lutris_install
    ...
    if (_HOME / ".var" / "app" / FAUGUS_FLATPAK_ID).is_dir():
        try:
            from Utils.executables.launch import game_is_faugus_install
...
def ensure_launcher_handoff_access(game, *, log_fn: LogFn) -> None:
    ...
    if not getattr(game, "native_launch_required", False):
        return
    try:
        from Utils.launchers.handoff import flatpak_launcher_app_for_handoff
