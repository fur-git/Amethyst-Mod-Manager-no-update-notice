...
from Utils.processes.sleep import inhibit_sleep
from Utils.environment.xdg import spawn_watched
...
def load_custom_exes(game) -> list[Path]:
    ...
    from Utils.profiles.state import read_custom_exes, read_profile_state
...
def save_custom_exes(game, paths: list[Path]) -> None:
    ...
    if pdir is None:
        return
    from Utils.profiles.state import write_custom_exes
...
def detect_framework_exes(game, framework_states: "dict | None" = None) -> list[Path]:
    ...
    *framework_states* is an optional {label: STATE_*} map - the framework
    ...
    if game_path is None:
        return []
    from Utils.games.frameworks import STATE_NOT_DEPLOYED, resolve_file_ci
...
def _without_amethyst_steam_handoff(options: str, game) -> str | None:
    ...
    try:
        from Utils.launchers.handoff import launch_handoff_script_path
...
def heroic_app_names_for_launch(game) -> list:
    ...
    from Utils.launchers.heroic import find_heroic_app_name_by_exe
...
def game_is_heroic_install(game) -> bool:
    ...
    if not app_names:
        return False
    from Utils.launchers.heroic import find_heroic_launch_info
    try:
...
def lutris_slugs_for_launch(game) -> list:
    ...
    from Utils.launchers.lutris import find_lutris_slugs_by_exes
    exe_names = [getattr(game, "exe_name", None)]
    exe_names += list(getattr(game, "exe_name_alts", []) or [])
    try:
...
def faugus_gameids_for_launch(game) -> list:
    ...
    from Utils.launchers.faugus import find_faugus_gameids_by_exes
    exe_names = [getattr(game, "exe_name", None)]
    exe_names += list(getattr(game, "exe_name_alts", []) or [])
    try:
...
def spawn_process_watched(cmd: list, *, env: "dict | None" = None,
    ...
    from Utils.environment.xdg import strip_sandbox_display_env
...
def launch_via_steam(steam_id: str, log_fn=_noop_log,
    ...
    *extra_args* are forwarded to the game's command line. Two forms, tried
    ...
    run_id = str(steam_id)
    try:
        from Utils.launchers.steam_shortcuts import is_shortcut_appid, run_gameid
...
def launch_via_lutris(slugs: list, log_fn=_noop_log,
    ...
    if lutris_is_flatpak:
        ...
        from Utils.ui.config import load_lutris_appimage_path
...
def launch_via_faugus(gameids: list, log_fn=_noop_log) -> bool:
    ...
    from Utils.launchers.faugus import FAUGUS_FLATPAK_ID, find_faugus_launch_info
    try:
    ...
    if faugus_is_flatpak:
        ...
        from Utils.ui.config import load_faugus_appimage_path
...
def get_tool_prefix_env(
    ...
    *steam_id* is accepted for call-site symmetry but intentionally NOT used to
    ...
    if is_new:
        try:
            from Utils.launchers.steam import steam_client_installed
            subprocess.run(
...
def get_game_prefix_env(game, log_fn=_noop_log, *,
    ...
    if proton_script is None and allow_runner_fallback:
        from Utils.wine.prefix import read_prefix_runner
...
WINEPREFIX + Proton's bin on PATH, ``wine start.exe <exe>``), with only two
    ...
    *extra_env* is applied last, a value of ``None`` removes the variable.
    ...
    if wine_bin is None:
        from Utils.wine.protontricks import _get_proton_bin
...
def launch_wine_tool_in_prefix(proton_script: Path, prefix_dir: Path, env: dict,
    ...
    *proton_script*/*env* come from ``get_tool_prefix_env`` (env already carries
    ...
    Utils.wine.proton.wine_tool_command: uses Proton's ``runinprefix`` verb (running
    ...
    """
    from Utils.wine.proton import _host_forward
    from Utils.launchers.steam import proton_run_command
...
def launch_game(game, log_fn=_noop_log) -> None:
    ...
    if steam_id:
        try:
            from Utils.launchers.steam_shortcuts import is_shortcut_appid
...
def epic_args_for_game(game, log_fn=_noop_log) -> list:
    ...
    if not app_names:
        return []
    try:
        from Utils.launchers.heroic import epic_launch_args, find_heroic_launch_info
...
def launch_exe_via_proton(
    ...
    from Utils.launchers.heroic import redact_epic_args
...
def launch_jar(jar_path: Path, game, log_fn=_noop_log) -> None:
    ...
    if runtime == JAR_RUNTIME_PROTON:
        ...
        from Utils.wine.jre import java_exe_in_prefix, JAVA_EXE_WIN
