...
def _lutris_root_candidates() -> list[LutrisRoot]:
    ...
    try:
        from Utils.ui.config import load_lutris_data_path
...
def _stored_exe_matches(stored_exe: str, rel_parts: list[str]) -> bool:
    ...
    exe name segments (same rules as heroic._stored_exe_matches)."""
...
def build_installed_exe_index() -> list[list[str]]:
    ...
    one-pass Steam/Heroic indexes in ``Utils.launchers.installed``). Games with a
...
def find_umu_run() -> Path | None:
    ...
    try:
        from Utils.launchers.umu import bundled_umu_run
