...
def _faugus_root_candidates() -> list[FaugusRoot]:
    ...
    try:
        from Utils.ui.config import load_faugus_data_path
...
def build_installed_exe_index() -> list[list[str]]:
    ...
    indexes in ``Utils.launchers.installed``)."""
    out: list[list[str]] = []
...
def find_faugus_proton_for_prefix(prefix_path: "str | Path") -> Path | None:
    ...
    if not name:
        return None
    try:
        from Utils.launchers.steam import list_installed_proton, _normalize_tool_name
