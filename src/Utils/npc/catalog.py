"""Catalog baked FaceGen heads using the NPC records that define them.
...
catalogue to NPC_ records so the row reads "Nazeem" instead:
    ...
    meshes/actors/character/facegendata/facegeom/<plugin>/<formid>.nif
        ...
        -> FULL (localised: a string id, see Utils.bethesda.strings) -> name

Every copy is listed, winner-flagged by Utils.assets.catalog, so a contested head
...
from Utils.npc.facegen import _parse_cached, facegen_npc
from Utils.assets.catalog import (DEFAULT_EXTS, MOD_ARCHIVE, MOD_LOOSE,
                                _enabled_mods, build_catalog, source_label)
from Utils.bethesda.strings import load_tables, resolve
