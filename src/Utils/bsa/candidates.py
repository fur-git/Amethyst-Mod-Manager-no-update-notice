"""Assess and rank mods for packing into a BSA or BA2.
...
GUI-free by design (see the sibling ``plugin_audit_core`` / ``Utils.bethesda.skygen``):
...
from Utils.archives.rules import is_packable, texture_extensions_for_game
from Utils.bsa.pack import (
...
# archive past 4 GiB is unreadable either way (writer.py raises there). The
# per-file size field is 30 bits, and the writer raises rather than skipping the
    ...
    for i, mod_name in enumerate(enabled):
        ...
        for record in normal:
            ...
            if record.namespace == "root":
                continue
            # A BSA stores everything under a folder; writer._collect_files
